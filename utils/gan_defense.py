...
def model_eval_gan(
    ...
    for batch in range(nb_batches):
        ...
        X_cur[:cur_batch_size] = test_images[start:end]
        Y_cur[:cur_batch_size] = test_labels[start:end]

        # The last batch may be smaller than all others, so we need to
        # account for variable batch size here.
        feed_dict = {images: X_cur, labels: Y_cur}
        if feed is not None:
        ...
        if z_norm is not None:
            cur_znorm = outs[z_norm_ind]
            z_norms.append(cur_znorm)
        ...
        if debug:
            if batch < 25:
                img_vis = test_images[start:end]
                recons_vis = outs[-2]
                adv_vis = outs[-1]
                prefix = 'im_{}'.format(batch)
                save_images_files(img_vis, output_dir=vis_dir, labels=cur_labels_val, prefix=prefix, postfix='_orig')
                save_images_files(adv_vis, output_dir=vis_dir, labels=cur_labels_val, prefix=prefix, postfix='_adv')
                save_images_files(recons_vis, output_dir=vis_dir, labels=cur_preds_val, prefix=prefix, postfix='_rec')
