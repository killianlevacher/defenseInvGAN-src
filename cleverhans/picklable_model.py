...
class Linear(Layer):
  ...
  def set_input_shape(self, input_shape):
    ...
    if self.use_bias:
      self.b = PV(name=self.name + 'linear_bias', initializer=(np.zeros((self.num_hid,))
...
class Conv2D(Layer):
  ...
  def set_input_shape(self, input_shape):
    ...
    if self.init_mode == "norm":
      ...
      raise ValueError(self.init_mode)
    self.kernels = PV(name=self.name + "_kernels", initializer=init)
    if self.use_bias:
      self.b = PV(name=self.name+"_bias", initializer=np.zeros((self.output_channels,)).astype('float32'))
...
class GroupNorm(Layer):
  ...
  def set_input_shape(self, shape):
    ...
    self.beta = PV(name=self.name + "_beta", 
                   initializer= np.zeros((self.channels,), dtype='float32'),
                   )
